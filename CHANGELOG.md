...
- HVAC system diagrams reconstructed directly from an `IDFDocument` — the IDF-native analogue of the EnergyPlus HVAC-Diagram utility, with no simulation run required. `idfkit.visualization.build_hvac_graph(doc)` walks `AirLoopHVAC`/`PlantLoop`/`CondenserLoop` definitions, their branch lists, connectors, supply/return paths, and zone equipment to build a directed `HVACGraph` using the same node-name connectivity rule as `eplusout.bnd` (a component whose outlet is node N feeds the component whose inlet is node N). A water coil shared between an air supply branch and a plant demand branch resolves to a single vertex carrying both loop memberships. Compound container components (`AirLoopHVAC:UnitarySystem`, the unitary heat-pump/heat-cool family, and `CoilSystem:Cooling:*`/`CoilSystem:Heating:DX`) are expanded into their internal fan/coil train so a packaged unit reads as its sequence of components inside the loop rather than as an opaque box that duplicates the flow. Render to a Mermaid flowchart (`to_mermaid()` / `hvac_to_mermaid()`), Graphviz DOT (`to_dot()` / `hvac_to_dot()`), or a JSON-serializable dict (`to_dict()` / `to_json()`); output is deterministic. The document must be expanded first — `build_hvac_graph` raises the new `HVACDiagramError` when `HVACTemplate:*` objects remain, unless `expand=True` is passed. Building is total otherwise, recording dangling-node and unconnected-component issues as `HVACWarning`s on the graph. For large models (a hospital or high-rise has dozens of loops and hundreds of components), `HVACGraph.subset(loop_names=..., loop_types=...)` filters the graph to selected loops (both filters matched case-insensitively), keeping each served zone's full conditioning train — including zone-only equipment with no loop membership (fan-coil fans, VRF terminal coils) and the refrigerant links to their outdoor unit — so a kept zone is never rendered as a bare box, and `subset()` with no arguments returns an equivalent graph. `HVACGraph.overview_mermaid()` collapses the whole building to one node per loop and zone (with plant→air-loop coupling and the zones each loop serves). A whole-building model has so many subgraphs that Mermaid's default `dagre` layout fails to render it at all, so `HVACDiagramConfig(layout="elk")` switches to the ELK engine (which handles them), `HVACGraph.is_complex` flags such models, the component render prepends a hint pointing at these tools, and the Jupyter inline preview (`_repr_markdown_`) falls back to the overview rather than emitting a diagram the notebook cannot draw. The return-air leg (zone → return mixer/plenum) is drawn as a dashed edge so an air loop reads as a closed supply/return circuit (toggle with `HVACDiagramConfig(show_return_air=False)`). Pure stdlib, zero new dependencies.
- HVAC diagrams now represent zone equipment properly. Zone forced-air units (`ZoneHVAC:FourPipeFanCoil`, `ZoneHVAC:PackagedTerminalAirConditioner`/`:HeatPump`, `ZoneHVAC:UnitVentilator`, `ZoneHVAC:TerminalUnit:VariableRefrigerantFlow`, …) are expanded into their internal outdoor-air-mixer/fan/coil train — like the existing unitary-system containers — and grouped under the zone they serve instead of a flat "Other equipment" bin. The expandable-container set is driven by the schema object group (`"Zone HVAC Forced Air Units"`), so new zone-unit types are picked up automatically rather than via a hardcoded list. Variable-refrigerant-flow systems now draw the refrigerant network: the `AirConditioner:VariableRefrigerantFlow` outdoor unit links to each terminal unit's DX coils through its `ZoneTerminalUnitList`, exposed as `HVACGraph.refrigerant_edges` (a new `HVACRefrigerantEdge`) and rendered as dashed `refrigerant` edges into a dedicated "VRF refrigerant system" cluster. Single-loop air-terminal reheat coils (electric/gas) now attach to their terminal's air-loop demand side instead of floating unconnected. Boundary-node warning noise dropped sharply (outdoor-air and relief nodes on `OutdoorAir:Mixer` are recognised as system boundaries), and the false "unconnected component" warnings that compound-unit boxes used to raise are gone. ([#172](https://github.com/idfkit/idfkit/pull/172))

### Fixed

- HVAC-diagram node detection missed components whose air-node fields use a bare `_node` suffix (e.g. VRF DX coils' `coil_air_inlet_node`) rather than `_node_name`, and `child_component_refs` failed to expand a container whose child fields are paired as `<prefix>_object_type`/`<prefix>_object_name` (VRF terminal units) or whose child type value is miscased (`COIL:...`). Both are resolved, so VRF terminal-unit coils and fan-coil internals now appear in the diagram. ([#172](https://github.com/idfkit/idfkit/pull/172))
