...
Read `references/<topic>.md` for focused, task-oriented guidance. Each reference is self-contained and includes runnable code examples mined from the linted `agent_references/snippets/` directory and the test suite. When in doubt, fetch the smallest reference that covers your task — don't pre-load everything.
...
- **Code snippets in references are mined from `agent_references/snippets/`** — they are linted with `ruff` and type-checked with `pyright`, so they run as-is.
