...
class TestParseVersionSpecDeadBranch:
    """Cover the unreachable else-branch at the end of _parse_version_spec.

    The guard that rejects non-{2,3}-part specs makes the final raise dead code.
    We reach it by patching ``len`` at module scope so that ``len(values)``
    returns 4, bypassing both ``if len(values) == 2`` and ``if len(values) == 3``.
    """

    def test_dead_branch_via_len_patch(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """Patch module-level ``len`` so ``len(values)`` returns 4, triggering the dead raise."""
        import argparse
        import builtins
        ...
        def fake_len(obj: object) -> int:
            if isinstance(obj, tuple) and all(isinstance(x, int) for x in obj):  # type: ignore[union-attr]
                return 4
            return real_len(obj)  # type: ignore[arg-type]

        monkeypatch.setattr(cli_module, "len", fake_len, raising=False)  # pyright: ignore[reportPrivateUsage]

        with pytest.raises(argparse.ArgumentTypeError, match="Invalid version specifier"):
            cli_module._parse_version_spec("24.1")  # pyright: ignore[reportPrivateUsage]


class TestCheckerUnknownLiteralKind:
    """Cover the branch in check_compatibility where literal.kind is neither OBJECT_TYPE nor CHOICE_VALUE."""

    def test_unknown_literal_kind_skipped(self) -> None:
        """A literal whose kind compares unequal to both known kinds is silently skipped."""
        from unittest.mock import MagicMock, patch

        from idfkit.compat._models import ExtractedLiteral

        class _UnknownKind:
            def __eq__(self, other: object) -> bool:
                return False

        mock_literal = MagicMock(spec=ExtractedLiteral)
        mock_literal.kind = _UnknownKind()
        mock_literal.obj_type = None
        mock_literal.value = "Zone"
        mock_literal.line = 1
        mock_literal.col = 0
        mock_literal.end_col = 4

        import idfkit.compat._checker as checker_module  # pyright: ignore[reportPrivateUsage]

        with patch.object(checker_module, "extract_literals", return_value=[mock_literal]):
            from idfkit.compat._checker import check_compatibility  # pyright: ignore[reportPrivateUsage]

            diagnostics = check_compatibility("x = 1", "test.py", targets=[(24, 1, 0), (24, 2, 0)])
            assert diagnostics == []
