...
coil = doc.add("Coil:Cooling:Water", "AHU-1 Cooling Coil",
...
fan = doc.add("Fan:VariableVolume", "AHU-1 Fan",
...
for coil in doc.get_collection("Coil:Cooling:Water"):
    ...
    upstream = [o for o in doc.all_objects()
                if getattr(o, "air_outlet_node_name", None) == inlet]
    downstream = [o for o in doc.all_objects()
                  if getattr(o, "air_inlet_node_name", None) == outlet]
